...
class GBIMemberController extends BaseController
{
   ...
   public function update(Request $request,$id){
        ...
        $validator = Validator::make($request->all(), ['name' => ['required',new AlphaSpace], 'gender' => 'required','address' => 'required|min:3', 'email' => ['required','email',new EmailValidate, Rule::unique('users')->ignore($user->id)],'phone_no' => ['required','numeric',new PhoneNubmerValidate], 'role_id' => 'required','department_id' => 'required', 'dob'=> 'required']);
        if ($validator->fails()) {
            return $this->errorValidate($validator->errors());
        }
        try{
